...
class Option(object):
	...
	def crank_nicolson(self, smax, smin, tmax, nsteps, ntimes):
		...
		returns:
			implied volatility
		"""
		hz = (smax - smin) / (2.0 * nsteps) # stock stepsize
		tz = (tmax) / float(ntimes) # time stepsize  	
		# set up the matrix
		mat = np.zeros(shape = (nsteps, ntimes))
	...
	def cn_implied_vol(self, x0, tol, niter):
		...
		while abs(sigmatemp - cn_est) > tol:
			print('while loop - cn price is {}'.format(cn_est))
			cn_est = sigmatemp
	...
	def d1(self):
		...
		"""
		return (math.log(self.s/self.k) + self.t * (self.r - self.q + self.sigma**2 / 2.0)) / (self.sigma * math.sqrt(self.t))
